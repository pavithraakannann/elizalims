/* --------------------------------------------------
  * © Copyright 2025 - ElizaLIMS - AI Powered Laboratory Automation Platform  by Designesia
  * --------------------------------------------------*/
(function($) {
	'use strict';
    ...
    var header = $("header");
      ...
      * --------------------------------------------------*/
     function menu_arrow() {
         console.log('Initializing menu_arrow');
         
         // Track if we're in mobile view
         var isMobile = $(window).width() <= 992;
         
         // First, ensure all submenu items have the correct classes
         jQuery('#mainmenu > li').each(function() {
             var $this = $(this);
             if ($this.find('> ul').length > 0) {
                 $this.addClass('has-submenu');
                 var $link = $this.find('> a');
                 $link.addClass('has-submenu-link');
                 
                 // Add arrow icon if not already present
                 if ($link.find('.mobile-arrow').length === 0) {
                     $link.append('<span class="mobile-arrow"><i class="fa fa-angle-down"></i></span>');
                 }
                 
                 // Ensure the submenu is initially hidden on mobile
                 if (isMobile) {
                     $this.find('> ul').hide();
                 }
             }
         ...
         function closeAllSubmenus() {
             $('#mainmenu > li.has-submenu').removeClass('active')
                 .find('> ul').slideUp(200)
                 .end()
                 .find('.mobile-arrow i').removeClass('fa-rotate-180');
         }
         
         // Handle clicks on menu items with submenus
         jQuery(document).off('click', '#mainmenu > li.has-submenu > a').on('click', '#mainmenu > li.has-submenu > a', function(e) {
             if (!isMobile) return true; // Let default behavior on desktop
             ...
             var isSubmenuVisible = $parentLi.hasClass('active');
             
             console.log('Menu item clicked', isSubmenuVisible);
             
             // If submenu is visible and link is clickable, navigate to the link
             if (isSubmenuVisible && $this.attr('href') && $this.attr('href') !== '#') {
             ...
             if (isSubmenuVisible) {
                 ...
                 $this.find('.mobile-arrow i').addClass('fa-rotate-180');
                 
                 // Push to history to handle back button
         ...
         $(window).on('popstate', function(event) {
             ...
             var $activeMenu = $('#mainmenu > li.has-submenu.active');
             if ($activeMenu.length) {
                 closeSubmenu($activeMenu);
                 
                 // Prevent default back behavior if we just closed a submenu
                //  if (event.originalEvent && event.originalEvent.state && event.originalEvent.state.menuOpen) {
    ...
    function de_share(){
     ...
     $(function(){
        $('#de-loader ').prepend($('<div class="lds-roller"><div></div><div></div><div></div><div></div><div></div><div></div><div></div><div></div></div>'));
         ...
         de_sidebar();
         de_share();
         menu_arrow(); // Initialize mobile menu functionality
         $(".jarallax").jarallax();
...
$('#mainmenu a').on('click', function () {
    // Only close the mobile menu if the link is not "#" or doesn't have submenu
    var href = $(this).attr('href');
    var hasSubmenu = $(this).next('ul').length > 0;

    if ($(window).width() <= 992 && href !== '#' && !hasSubmenu) {
